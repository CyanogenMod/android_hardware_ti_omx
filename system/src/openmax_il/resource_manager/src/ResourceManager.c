...
int main()
{
    ...
    printf("Warning - using stub version of Resource Manager!!\n");
#endif
    ...
    if((mknod(RM_SERVER_IN,S_IFIFO|PERMS,0)<0)&&(rm_errno!=EEXIST)) 
                ...
                switch (cmd_data.RM_Cmd) {  
                    ...
                    case RM_OpenPipe:
                        ...
                        if((fdwrite=open(rmsideNamedPipeName,O_WRONLY))<0) {
                            RM_DPRINT("[Resource Manager] - failure to open the WRITE pipe\n");
                        }
                        else {
            ...
            switch(policyresponse_data.PM_Cmd) {
                ...
                case PM_DENYPOLICY:
                    ...
                    if(write(RM_GetPipe(globalrequest_cmd_data.hComponent,globalrequest_cmd_data.nPid), &globalrequest_cmd_data, sizeof(globalrequest_cmd_data)) < 0)
                        ...
                        RM_DPRINT ("[Resource Manager] - ok to write data back to component\n");
...
OMX_ERRORTYPE InitializeQos()
{
    ...
    if (qosdllname == NULL)
    {
        ...
        printf ("[Resource Manager] - No QosDin DLL file name\n");
        return eError;
    }
    ...
    if (DSP_SUCCEEDED(status)) {
        ...
        if (DSP_SUCCEEDED(status)) {
            ...
            status = DSPManager_RegisterObject(
                    ...
                    qosdllname);
            if (DSP_FAILED(status)) {
                printf ("[Resource Manager] - InitializeQos() -- DSPManager_RegisterObject() object fail=0x%x\n", (unsigned int)status);
                eError= OMX_ErrorHardware;
            }
        }
        else
    ...
    {
        printf ("[Resource Manager] - InitializeQos() -- DSPProcessor_Attach() object fail=0x%x\n", (unsigned int)status);
        eError= OMX_ErrorHardware;
...
void *RM_CPULoadThread(int pipeToWatch)
{
    ...
    while (1) {
        ...
        switch (op) {
            ...
            default:
                fprintf(stderr,"RM read incorrect operating point\n");
            break;
        }
