...
*
* This file implements resource manager for Linux 8.x that
...
OMX_ERRORTYPE RMProxy_NewInitalizeEx(OMX_LINUX_COMPONENTTYPE componetType)
{
    ...
    case OMX_COMPONENTTYPE_VIDEO:
        ...
        {
            printf("[Resource_Manager_Proxy] - RMProxy_Init - more than %d instances\n", MAX_VIDEO_INSTANCES);
            eError = OMX_ErrorInsufficientResources;
            nVideoInstances--;
            goto EXIT;
        }
        ...
        {
            printf("[Resource_Manager_Proxy] - RMProxy_Init - more than 2 instances\n");
            eError = OMX_ErrorInsufficientResources;
            nVPPInstances--;
            goto EXIT;
        }
    ...
    default:
        printf("[Resource_Manager_Proxy] - RMProxy_Init - Invalid Component Type\n");
...
OMX_ERRORTYPE RMProxy_DeinitalizeEx(OMX_LINUX_COMPONENTTYPE componentType)
{
    ...
    default:
        printf("[Resource_Manager_Proxy] - RMProxy_DeInit - Invalid Component Type\n");
        eError = OMX_ErrorBadParameter;
        goto EXIT;
        break;
    }
    ...
    {
        /*printf("[Resource_Manager_Proxy] - do nothing with nInstances=%d\n", nInstances);*/
    }
    EXIT:
    return (OMX_ErrorNone);
}
            ...
            /* add component and corresponding callback to the list */
                ...
                if (numClients == 0) {
                    if ((RMProxyfdwrite=open(RM_SERVER_IN, O_WRONLY))<0)
                        RMPROXY_DPRINT("[Resource_Manager_Proxy] - failure to open WRITE pipe [RMProxyfdwrite=%d], [rmp_errno=%d]\n", RMProxyfdwrite, rmp_errno);
                  ...
                  /*  RMProxy_itoa((int)cmd_data.hComponent,handleString);*/
                    RMProxy_itoa((int)getpid(),handleString);
                    ...
                    if((mknod(namedPipeName,S_IFIFO|PERMS,0)<0)&&(rmp_errno!=EEXIST))
                    ...
                    if ((RMProxyfdread=open(namedPipeName, O_RDONLY ))<0)
                        RMPROXY_DPRINT("[Resource_Manager_Proxy] - failure to open READ pipe [RMProxyfdread=%d], [rmp_errno=%d]\n", RMProxyfdread, rmp_errno);
                    readPipeOpened = 1;
