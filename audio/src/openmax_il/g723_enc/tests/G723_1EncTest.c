...
* =========================================================================== */
/**
* @file G723_1Enc_Test.c
*
* This file implements G723_1 Encoder Component Test Application to verify
* which is fully compliant with the Khronos OpenMAX (TM) 1.0 Specification
*
* @path  $(CSLPATH)\OMAPSW_MPU\linux\audio\src\openmax_il\g723_enc\tests
*
* @rev  1.0
*/
/* ----------------------------------------------------------------------------
...
/* ======================================================================= */
/**
 * @def G723_1ENC_INPUT_BUFFER_SIZE        Default input buffer size
 */
/* ======================================================================= */
 ...
 */
/* ======================================================================= */
#define G723_1ENC_OUTPUT_BUFFER_SIZE 24 /*Component default output buffer size*/
...
OMX_STRING strG723_1Encoder = "OMX.TI.G723_1.encode";
...
int nRead = 0;
int DasfMode =0;
OMX_U8 NextBuffer[G723_1ENC_INPUT_BUFFER_SIZE*3] = {0};
OMX_U32 fdwrite = 0;
OMX_U32 fdread = 0;  
...
{
    OMX_CALLBACKTYPE G723_1CaBa = {(void *)EventHandler,
                (void*)EmptyBufferDone,
                                (void*)FillBufferDone};
    OMX_HANDLETYPE pHandle;
    OMX_ERRORTYPE eError = OMX_ErrorNone;
    OMX_U32 AppData = G723_1ENC_APP_ID;
    OMX_PARAM_PORTDEFINITIONTYPE* pCompPrivateStruct = NULL;
    OMX_AUDIO_PARAM_G723TYPE *pG723_1Param = NULL;
    ...
    printf("------------------------------------------------------\n");
    printf("This is Main Thread In G723_1 ENCODER Test Application:\n");
  ...
  printf("Line %d\n",__LINE__);
    ...
    if(argc < 11) {
        printf("%d :: Usage: [TestApp][Input File][Output File][FUNC_ID_X][FM/DM][ACDN_ON/ACDN_OFF][5.3/6.3][DTX_ON/DTX_OFF][HiPassFilter/PostFilter][# INPUT BUF][# OUTPUT BUF][# REP]\n",__LINE__);
        goto EXIT;
    }
    ...
    printf("%d :: App: g723_1 output file is %s\n", __LINE__, argv[2]);
    fOut = fopen(argv[2], "w");
    if( fOut == NULL ) {
    ...
    for(j = 0; j < testcnt1; j++) {
        ...
        if(j >= 1) {
            ...
            fOut = fopen("TC6_G723_11.G723_1", "w");
            if( fOut == NULL ) {
        ...
        /* Load the G723_1 Encoder Component */
    #ifdef OMX_GETTIME
        GT_START();
        eError = TIOMX_GetHandle(&pHandle, strG723_1Encoder, &AppData, &G723_1CaBa);
        ...
        OMX_NBAPP_MALLOC_STRUCT(pG723_1Param, OMX_AUDIO_PARAM_G723TYPE);
        OMX_NBAPP_CONF_INIT_STRUCT(pG723_1Param, OMX_AUDIO_PARAM_G723TYPE);
        ...
        pCompPrivateStruct->nBufferSize                        = G723_1ENC_INPUT_BUFFER_SIZE;
        ...
        if(!(strcmp(argv[4],"FM"))) {
            ...
            DasfMode = 1;
            APP_DPRINT("\n%d :: App: audioinfo->dasfMode = %ld \n",__LINE__,audioinfo->dasfMode);
            APP_DPRINT("%d :: G723_1 ENCODER RUNNING UNDER DASF MODE \n",__LINE__);
        ...
        if(audioinfo->dasfMode) { /*Dasf Mode*/
            if((atoi(argv[9])) != 0) {
                eError = OMX_ErrorBadParameter;             
                printf("%d :: App: For DASF mode argv[9] Should Be --> 0\n",__LINE__);
                goto EXIT;
            }
        } else {
            if((atoi(argv[9])) == 0) {  /*File Mode*/
                eError = OMX_ErrorBadParameter;
                printf("%d :: App: For FILE mode argv[9] Should be greater than zero depends on number of buffers user want to encode\n",__LINE__);
                goto EXIT;
            }
        }
        ...
        pCompPrivateStruct->nBufferSize                        = G723_1ENC_OUTPUT_BUFFER_SIZE;
        pCompPrivateStruct->bEnabled                           = OMX_TRUE;
        pCompPrivateStruct->bPopulated                         = OMX_FALSE;
        pCompPrivateStruct->eDomain                            = OMX_PortDomainAudio;
        pCompPrivateStruct->format.audio.eEncoding             = OMX_AUDIO_CodingG723;
        ...
        if(!(strcmp(argv[6],"5.3"))) {
              pG723_1Param->eBitRate = OMX_AUDIO_G723ModeLow;
              APP_DPRINT("\n%d :: App: eBitRate set to 5300 bps\n",__LINE__);
        } 
        else if(!(strcmp(argv[6],"6.3"))) {
              pG723_1Param->eBitRate = OMX_AUDIO_G723ModeHigh;
              APP_DPRINT("\n%d :: App: eBitRate set to 6300 bps\n",__LINE__);
        }
        else{
              pG723_1Param->eBitRate = OMX_AUDIO_G723ModeHigh;
              APP_DPRINT("\n%d :: App: UnSuported bit Rate, setting 6300 bps as default\n",__LINE__);
         }

        if(!(strcmp(argv[7],"DTX_ON"))) {
              pG723_1Param->bDTX = OMX_TRUE;
              APP_DPRINT("\n%d :: App: Setting Voice Activity detection to ON\n",__LINE__);
        } 
        else if(!(strcmp(argv[7],"DTX_OFF"))) {
              pG723_1Param->bDTX = OMX_FALSE;
              APP_DPRINT("\n%d :: App: Setting Voice Activity detection to OFF\n",__LINE__);
        }
        else{
              pG723_1Param->bDTX = OMX_TRUE;
              APP_DPRINT("\n%d :: App: No valid option found, setting Voice Activity detection to ON\n",__LINE__);
         }

        if(!(strcmp(argv[8],"HiPassFilter"))) {
              pG723_1Param->bHiPassFilter = OMX_TRUE; /**< Enable High Pass Filter */ 
              pG723_1Param->bPostFilter = OMX_FALSE;   /**< Disable Post Filter */   
              APP_DPRINT("\n%d :: App: Enabling High Pass Filter\n",__LINE__);
        } 
        else if(!(strcmp(argv[8],"PostFilter"))) {
              pG723_1Param->bHiPassFilter =OMX_FALSE; /**< Disable High Pass Filter */ 
              pG723_1Param->bPostFilter = OMX_TRUE;   /**< Enable Post Filter */   
              APP_DPRINT("\n%d :: App: Enabling Post Filter\n",__LINE__);
        }
        else{
              pG723_1Param->bHiPassFilter = OMX_TRUE; /**< Enable High Pass Filter */ 
              pG723_1Param->bPostFilter = OMX_FALSE;   /**< Disable Post Filter */   
              APP_DPRINT("\n%d :: App: No valid option found, setting High Pass Filter as default\n",__LINE__);
         }

        /* Send  G723_1 config for output */
        pG723_1Param->nSize                    = sizeof(OMX_AUDIO_PARAM_G723TYPE);
        pG723_1Param->nVersion.s.nVersionMajor = 0xF1;
        pG723_1Param->nVersion.s.nVersionMinor = 0xF2;
        pG723_1Param->nPortIndex               = OMX_DirOutput;
        pG723_1Param->nChannels                = NBAPP_NUM_OF_CHANNELS;
    #ifdef OMX_GETTIME
        GT_START();
        eError = OMX_SetParameter (pHandle, OMX_IndexParamAudioG723, pG723_1Param);
        GT_END("Set Parameter Test-SetParameter");
    #else
        eError = OMX_SetParameter (pHandle, OMX_IndexParamAudioG723, pG723_1Param);
    #endif
    ...
    if (audioinfo->dasfMode){
          if((fdwrite=open(FIFO1,O_WRONLY))<0) {
               APP_DPRINT("%d :: [G723_1E Component] - failure to open WRITE pipe\n",__LINE__);
          }
          if((fdread=open(FIFO2,O_RDONLY))<0) {
               APP_DPRINT("%d :: [G723_1E Component] - failure to open READ pipe\n",__LINE__);
          }
         ...
         if((write(fdwrite, &cmd_data, sizeof(cmd_data)))<0) {
             APP_DPRINT("%d :: [G723_1E Component] - send command to audio manager\n",__LINE__);
         }
         if((read(fdread, &cmd_data, sizeof(cmd_data)))<0) {
             printf("%d :: [G723_1E Component] - failure to get data from the audio manager!!! \n",__LINE__);
             goto EXIT;
         }
         if(cmd_data.streamID == 0) {
             printf("%d :: [G723_1E Component] - no input stream available!!!!!!!\n",__LINE__);
             eError = OMX_ErrorInsufficientResources;
              goto EXIT;
        
         } else {
               APP_DPRINT("%d :: [G723_1E Component] - input stream available\n",__LINE__);
    ...
    eError = OMX_GetExtensionIndex(pHandle, "OMX.TI.index.config.G723_1.datapath",&index);
    if (eError != OMX_ErrorNone) {
    ...
    if(eError != OMX_ErrorNone) {
        ...
        if (audioinfo->dasfMode ==  1) { 
            ...
            eError = OMX_GetExtensionIndex(pHandle, "OMX.TI.index.config.g723_1.streamIDinfo",&index);
            if (eError != OMX_ErrorNone) {
                printf("Error getting extension index\n");
                goto EXIT;
            }
        ...
        printf("%d Sending Component to OMX_StateIdle\n",__LINE__);        
    #ifdef OMX_GETTIME
        GT_START();
    #endif
        ...
        for(i = 0; i < numInputBuffers; i++) {
            ...
            eError = OMX_AllocateBuffer(pHandle, &pInputBufferHeader[i], 0, NULL, G723_1ENC_INPUT_BUFFER_SIZE*3);
            if(eError != OMX_ErrorNone) {
        ...
        for(i = 0; i < numOutputBuffers; i++) {
            ...
            eError = OMX_AllocateBuffer(pHandle, &pOutputBufferHeader[i], 1, NULL, G723_1ENC_OUTPUT_BUFFER_SIZE*3);
            if(eError != OMX_ErrorNone) {
...
#else
        for(i = 0; i < numInputBuffers; i++) {
            pInputBuffer[i] = (OMX_U8*)SafeMalloc(G723_1ENC_INPUT_BUFFER_SIZE);
            ...
            eError = OMX_UseBuffer(pHandle, &pInputBufferHeader[i], 0, NULL, G723_1ENC_INPUT_BUFFER_SIZE, pInputBuffer[i]);
            if(eError != OMX_ErrorNone) {
        ...
        for(i = 0; i < numOutputBuffers; i++) {
            pOutputBuffer[i] = SafeMalloc(G723_1ENC_OUTPUT_BUFFER_SIZE + 256);
            ...
            /* allocate output buffer */
            APP_DPRINT("%d :: About to call OMX_UseBuffer\n",__LINE__);
            eError = OMX_UseBuffer(pHandle, &pOutputBufferHeader[i], 1, NULL, G723_1ENC_OUTPUT_BUFFER_SIZE, pOutputBuffer[i]);
            if(eError != OMX_ErrorNone) {
                APP_DPRINT("%d :: Error returned by OMX_UseBuffer()\n",__LINE__);
                goto EXIT;
            }
    ...
    #endif
        if(eError != OMX_ErrorNone) {
            APP_DPRINT( "Error:  hG723_1Encoder->WaitForState reports an eError %X\n", eError);
            goto EXIT;
        }
        printf("%d Component on OMX_StateIdle\n",__LINE__);
        ...
        for(i = 0; i < testcnt; i++) {
            ...
            if(i >= 1) {
                ...
                fOut = fopen("TC5_G723_11.G723_1", "w");
                if(fOut == NULL) {
                    fprintf(stderr, "Error:  failed to create the output file %s\n", argv[2]);
                    goto EXIT;
                }
            ...
            printf("%d Sending Component to OMX_Executing\n",__LINE__);
        #ifdef OMX_GETTIME
            GT_START();
        #endif
        ...
        #endif
            printf("%d Component on OMX_Executing\n",__LINE__);
            if(eError != OMX_ErrorNone) {
                APP_DPRINT( "Error:  hG723_1Encoder->WaitForState reports an eError %X\n", eError);
                goto EXIT;
            }
            ...
            if(audioinfo->dasfMode == 0) {
                for (k=0; k < numInputBuffers; k++) {
                    ...
                    pBuffer->nFlags=0;
                            printf("%d About to call send_input_buffer\n",__LINE__);/*glen*/
                            #ifdef OMX_GETTIME
                                if (k==0)
                                { 
            ...
            for (kk = 0; kk < numOutputBuffers; kk++) {
                ...
                printf("%d About to call FillThisBuffern \n",__LINE__);/*glen*/
            #ifdef OMX_GETTIME
                if (kk==0)
                { 
            ...
            #endif
                OMX_FillThisBuffer(pComponent, pOutputBufferHeader[kk]);
            }
            ...
            if(audioinfo->dasfMode == 0) {
                if(FD_ISSET(IpBuf_Pipe[0], &rfds)) {                                           
                    ...
                    if (frmCount==15 && tcID ==3){ /*Pause the component*/
                        ...
                        sleep(3);
#ifdef FLUSHINPAUSE
                        printf("App: Sending Flush to both ports\n");
                    ...
                    if(nFrameCount == 50 && tcID == 7)                                                   {
                            ...
                            printf("[G723.1 encoder] --- will set stream gain to high\n");
                    ...
                    if(nFrameCount == 250 && tcID == 7)
                    {
                            ...
                            printf("[G723.1 encoder] --- will set stream gain to low\n");
...
/*-------*/
    ...
    for (i=0; i < numInputBuffers; i++) {
        /* allocate input buffer */
        eError = OMX_AllocateBuffer(pHandle,&pInputBufferHeader[i],0,NULL,G723_1ENC_INPUT_BUFFER_SIZE*3); /*To have enought space for    */
        if(eError != OMX_ErrorNone) {
...
/*-------*/                               
    ...
    goto SHUTDOWN;
            ...
            printf("%d :: App: G723_1 Encoded = %d Frames \n",__LINE__,(nOutBuff));
    ...
    #ifdef OMX_GETTIME
        GT_END("Call to SendCommand <OMX_StateLoaded> then portdisable & clear all Buffers");
    #endif
        if(eError != OMX_ErrorNone) {
            APP_DPRINT( "Error:  G723Encoder->WaitForState reports an error %X\n", eError);
            goto EXIT;
        }
...
SHUTDOWN: 
...
#endif          
        printf("%d :: App: Free the Component handle\n",__LINE__);
        /* Unload the G723_1 Encoder Component */
        eError = TIOMX_FreeHandle(pHandle);
        if((eError != OMX_ErrorNone)) {
            APP_DPRINT("%d :: Error in Free Handle function\n",__LINE__);
            goto EXIT;
        }
        printf("%d :: App: Free Handle returned Successfully\n",__LINE__);
        
        printf("%d :: App: Calling TIOMX_Deinit()\n",__LINE__);
        eError = TIOMX_Deinit();
        if(eError != OMX_ErrorNone) {
        ...
        printf("%d :: App: Freeing the Memory Allocated in TestApp\n",__LINE__);

        APP_MEMPRINT("%d :: App: [TESTAPPFREE] %p\n",__LINE__,pG723_1Param);
        if(pG723_1Param != NULL){
            SafeFree(pG723_1Param);
            pG723_1Param = NULL;
        }
    ...
    if (testcnt == MAX_CYCLES) {
        printf("%d :: *********************************************************************\n",__LINE__);
        printf("%d :: NOTE: An output file TC5_G723_11.G723_1 has been created in file system\n",__LINE__);
        printf("%d :: *********************************************************************\n",__LINE__);
    }
    if (testcnt1 == MAX_CYCLES) {
        printf("%d :: *********************************************************************\n",__LINE__);
        printf("%d :: NOTE: An output file TC6_G723_11.G723_1 has been created in file system\n",__LINE__);
        printf("%d :: *********************************************************************\n",__LINE__);
    }
EXIT:
#ifdef APP_DEBUGMEM    
...
OMX_ERRORTYPE send_input_buffer(OMX_HANDLETYPE pHandle, OMX_BUFFERHEADERTYPE* pBuffer, FILE *fIn)
{
    ...
    if(FirstTime){
        if(mframe == TWO_FRAMES){
            nRead = fread(pBuffer->pBuffer, 1, G723_1ENC_INPUT_BUFFER_SIZE*2, fIn);
        }
                else if(mframe == HALF_FRAME){
            nRead = fread(NextBuffer, 1, G723_1ENC_INPUT_BUFFER_SIZE/2, fIn);
            }
        else{
            nRead = fread(pBuffer->pBuffer, 1, G723_1ENC_INPUT_BUFFER_SIZE, fIn);
        }       
    ...
    if(mframe == TWO_FRAMES){
            ...
            nRead = fread(NextBuffer, 1, G723_1ENC_INPUT_BUFFER_SIZE, fIn);
    }   
    
    if(nRead < G723_1ENC_INPUT_BUFFER_SIZE && !DasfMode && (mframe != HALF_FRAME)){
        pBuffer->nFlags = OMX_BUFFERFLAG_EOS;
    }else{
          pBuffer->nFlags = 0;
          }
        
        if( ((mframe != HALF_FRAME) && (pBuffer->nFilledLen>= G723_1ENC_INPUT_BUFFER_SIZE)) || 
              ((mframe == HALF_FRAME) && (pBuffer->nFilledLen> 0))){
        
        if(pBuffer->nFlags == OMX_BUFFERFLAG_EOS){
                           printf("Sending Last Input Buffer from App\n");
        }
        /* printf("Sending %d bytes to Comp\n",(int) pBuffer->nFilledLen); */
...
#endif
    if(error != OMX_ErrorNone) {
                    fprintf(stderr, "\nError:  hG723Encoder->WaitForState reports an error %X!!!!!!!\n", error);
                    goto EXIT;
    }
EXIT:

#ifdef OMX_GETTIME
    GT_END("G723_ENC test <End>");
