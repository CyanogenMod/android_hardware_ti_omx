...
void* NBAMRENC_CompThread(void* pThreadData)
{
    ...
    while (1) {
        ...
        if (0 == status) {
            OMX_PRDSP4(pComponentPrivate->dbg, "%d :: bIsThreadstop = %ld\n",__LINE__,pComponentPrivate->bIsThreadstop);
            OMX_PRDSP4(pComponentPrivate->dbg, "%d :: lcml_nOpBuf = %ld\n",__LINE__,pComponentPrivate->lcml_nOpBuf);
            OMX_PRDSP4(pComponentPrivate->dbg, "%d :: lcml_nIpBuf = %ld\n",__LINE__,pComponentPrivate->lcml_nIpBuf);
            OMX_PRDSP4(pComponentPrivate->dbg, "%d :: app_nBuf = %ld\n",__LINE__,pComponentPrivate->app_nBuf);
            if (pComponentPrivate->bIsThreadstop == 1)  {
		    ...
		    OMX_ERROR4(pComponentPrivate->dbg, "%d :: pComponentPrivate->curState is not OMX_StateIdle\n",__LINE__);
             ...
             OMX_PRDSP4(pComponentPrivate->dbg, "%d :: Component Time Out !!!!! \n",__LINE__);
        } else if(-1 == status) {
		...
		goto EXIT;
            ...
            if (eError != OMX_ErrorNone) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: NBAMRENC_HandleDataBufFromApp returned error\n",__LINE__);
                break;
            }
        }
