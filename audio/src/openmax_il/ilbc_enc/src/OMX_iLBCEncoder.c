            ...
            OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    for (i=0; i < ILBCENC_MAX_NUM_OF_BUFS; i++) {
        ...
        if (buff == pBuffer) {
            ILBCENC_DPRINT("%d Found matching input buffer\n",__LINE__);
            ILBCENC_DPRINT("%d buff = %p\n",__LINE__,buff);
    ...
    for (i=0; i < ILBCENC_MAX_NUM_OF_BUFS; i++) {
        ...
        if (buff == pBuffer) {
            ILBCENC_DPRINT("%d Found matching output buffer\n",__LINE__);
            ILBCENC_DPRINT("%d buff = %p\n",__LINE__,buff);
            ILBCENC_DPRINT("%d pBuffer = %p\n",__LINE__,pBuffer);
            outputIndex = i;
            break;
    ...
    PERF_SendingBuffer(pComponentPrivate->pPERF,
        ...
        if (pComponentPrivate->pInputBufferList->numBuffers <
            pComponentPrivate->pPortDef[ILBCENC_INPUT_PORT]->nBufferCountMin) {

            pComponentPrivate->pPortDef[ILBCENC_INPUT_PORT]->bPopulated = OMX_FALSE;
            }
        if(pComponentPrivate->pPortDef[ILBCENC_INPUT_PORT]->bEnabled &&
    ...
    PERF_SendingBuffer(pComponentPrivate->pPERF,
        ...
        if (pComponentPrivate->pOutputBufferList->numBuffers <
            pComponentPrivate->pPortDef[ILBCENC_OUTPUT_PORT]->nBufferCountMin) {
            pComponentPrivate->pPortDef[ILBCENC_OUTPUT_PORT]->bPopulated = OMX_FALSE;
        }
        if( pComponentPrivate->pPortDef[ILBCENC_OUTPUT_PORT]->bEnabled &&
            pComponentPrivate->bLoadedCommandPending == OMX_FALSE &&
            (pComponentPrivate->curState == OMX_StateIdle ||
            pComponentPrivate->curState == OMX_StateExecuting ||
            pComponentPrivate->curState == OMX_StatePause)) {
        ...
        ILBCENC_EPRINT("%d Returning OMX_ErrorBadParameter\n",__LINE__);
        eError = OMX_ErrorBadParameter;
    }
