...
/* ================================================================================= * */
/**
* @fn G723DEC_ComponentThread() This is component thread that keeps listening for
...
void* G723DEC_ComponentThread (void* pThreadData)
{
    ...
    G723DEC_COMPONENT_PRIVATE* pComponentPrivate = (G723DEC_COMPONENT_PRIVATE*)pThreadData;
    OMX_COMPONENTTYPE *pHandle = pComponentPrivate->pHandle;
    int ret = 0;
    OMX_BUFFERHEADERTYPE *pBufHeader = NULL;

    G723DEC_DPRINT (":: Entering ComponentThread \n");
    ...
    while (1) {
        ...
        if (pComponentPrivate->bExitCompThrd == 1) {
            G723DEC_DPRINT(":: Comp Thrd Exiting here...\n");
            goto EXIT;
        }
        ...
        if (0 == status) {
            G723DEC_DPRINT("\n\n\n!!!!!  Component Time Out !!!!!!!!!!!! \n");
            if (pComponentPrivate->bExitCompThrd == 1) {
                G723DEC_EPRINT(":: Comp Thrd Exiting here...\n");
                goto EXIT;
            }

        } else if (-1 == status) {
            G723DEC_DPRINT (":: Error in Select\n");
            ...
            G723DEC_DPRINT (":: DATA pipe is set in Component Thread\n");
            ret = read(pComponentPrivate->dataPipe[0], &pBufHeader, sizeof(pBufHeader));
            if (ret == -1) {
                G723DEC_DPRINT (":: Error while reading from the pipe\n");
            }

            eError = G723DEC_HandleDataBuf_FromApp (pBufHeader,pComponentPrivate);
            if (eError != OMX_ErrorNone) {
                G723DEC_DPRINT (":: Error From HandleDataBuf_FromApp\n");
                break;
            }
        } else if (FD_ISSET (pComponentPrivate->cmdPipe[0], &rfds)) {
            G723DEC_DPRINT (":: CMD pipe is set in Component Thread\n");

            nRet = G723DEC_HandleCommand (pComponentPrivate);
            if (nRet == EXIT_COMPONENT_THRD) {
                G723DEC_DPRINT ("Exiting from Component thread\n");
...
EXIT:
    ...
    G723DEC_DPRINT (":: Exiting ComponentThread \n");
